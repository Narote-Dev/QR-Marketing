      ...
      <div className="flex items-center gap-2">
        <SignedOut>
          <SignInButton mode="redirect" forceRedirectUrl={generatorPath}>
            <button
              type="button"
              className="inline-flex min-h-11 items-center whitespace-nowrap rounded-lg px-3 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-50 hover:text-brand-teal-dark focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-teal md:min-h-8 md:px-2.5"
            >
              {signInLabel}
            </button>
          </SignInButton>
          <Link
            href={signUpPath}
            className="hidden h-8 items-center whitespace-nowrap rounded-lg px-2.5 text-sm font-medium text-slate-600 transition-colors hover:text-brand-coral sm:inline-flex"
          >
            {signUpLabel}
          </Link>
        </SignedOut>
