...
export function AdSlot({ placement, className = "", minHeight = 250 }: Props) {
  ...
  return <aside className={`my-8 w-full overflow-hidden ${className}`} aria-label={dictionary.chrome.advertisement} style={{ minHeight }}><ins className="adsbygoogle block" style={{ display: "block", minHeight }} data-ad-client={adSenseConfig.publisherId} data-ad-slot={slotId} data-ad-format="auto" data-full-width-responsive="true" /></aside>;
}
