...
export function TemplatePreview({ template }: Props) {
  // Step 1: Resolve empty-state and alt-text copy from the dictionary.
  const dictionary = useDictionary();
  const ui = dictionary.templatesUi;

  // Step 2: Show the selected template preview image, or an empty state.
  if (!template) {
    return (
  ...
  return (
    <div className="overflow-hidden rounded-2xl border bg-white shadow-sm">
      <div className="relative aspect-[4/5] bg-slate-50 sm:aspect-square">
        <Image src={template.previewImage} alt={formatMessage(ui.previewAlt, { name: template.name })} fill className="object-contain p-4" sizes="(max-width: 768px) 100vw, 320px" unoptimized />
      </div>
